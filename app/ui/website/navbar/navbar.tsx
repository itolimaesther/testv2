...
const Navbar = () => {
	...
	return (
		<nav className="flexBetween max-container padding-container bg-white relative z-30 py-5">
			<Link href="/home">
				<Image src="/vendulogo.svg" alt="logo" width={120} height={29} />
			</Link>
					...
					<div>
						{link.categories ? (
							<Menu as="div" className="relative inline-block text-left">
								<Menu.Button className="inline-flex justify-center w-full text-base text-gray-50 focus:outline-none border-0">
									{link.label}
									<ChevronDownIcon
				...
				<Dialog.Panel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
					<div className="flex items-center justify-between">
						<a href="#" className="-m-1.5 p-1.5">
							...
							<Image className="h-8 w-auto" src="/vendulogo.svg" alt="logo" />
						</a>
						<button
							type="button"
								...
								{NAV_LINKS.map((link) => (
									<div>
										{link.categories ? (
											...
											>
												<Menu.Button className="inline-flex py-2 justify-center text-gray-700 focus:outline-none border-0 text-base font-semibold leading-7 items-center">
													{link.label}
													<ChevronDownIcon
														className="ml-2 -mr-1 h-5 w-5 text-gray"
														aria-hidden="true"
